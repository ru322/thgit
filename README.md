...
# 特徴
一回セットアップを済ませれば特に意識することなく、Git連携できます。
...
デスクトップにショートカットが作成されるのでそこからゲームを起動してください。すでにショートカットが存在する場合は上書きされます。

※作品を追加したなどで、更新する必要がある場合は`.thgit-setup`を削除してから、`thgit.ps1`を起動してください

# 仕様上の注意
同期するファイルは`.gitignore`によって管理されています。現状`th06,th07,th08,th09`が対象です。対応していない作品についてもある程度容易に登録することが可能ですが、実行ファイルを含めてしまった場合、法律に触れる可能性があります。不用意に触らないことを強く推奨します。あくまで自己責任です。
